...
export default function Dashboard() {
    ...
    return (
        <div className={`min-h-screen transition-colors duration-700 ${sosActive ? 'bg-red-950 text-red-50' : 'bg-[#f4f7fb] text-slate-900'} p-4 md:p-8 font-sans`}>
            <div className="max-w-6xl mx-auto space-y-6">
                    ...
                    <div className="flex items-center gap-4 w-full md:w-auto">
                        ...
                        <div className="flex-1">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                                ...
                                <p className="text-xs text-slate-500 font-medium hidden sm:block">IoT Assistive Dashboard</p>
                            </div>
                        </div>
                    </div>
                    <div className="flex items-center gap-4 mt-4 md:mt-0 w-full md:w-auto justify-between md:justify-end">

                        <Link href="/profile" className="flex items-center gap-3 bg-[#f4f7fb] p-2 pr-4 rounded-full border border-slate-200 hover:bg-slate-200 transition-colors cursor-pointer group">
                            <img src="/patient.jpg" alt="Patient" className="w-10 h-10 rounded-full object-cover border-2 border-white shadow-sm group-hover:scale-105 transition-transform" />
                            <div className="flex flex-col">
                                <span className="text-xs font-bold text-slate-700 leading-tight group-hover:text-blue-600 transition-colors">Age: 67</span>
                                <span className="text-[10px] text-slate-500 leading-tight">Blood: A | Male</span>
                            </div>
                        </Link>
                        ...
                        <div className="flex flex-col items-end gap-2">
                            <div className="flex items-center gap-2 text-xs font-medium bg-white px-3 py-1.5 rounded-full shadow-sm border border-slate-100">
                                ...
                                <span className={error ? "text-rose-500" : "text-slate-600"}>
                                    {error ? "Offline" : `Synced: ${lastUpdated}`}
                                </span>
                            </div>
                            <button
                    ...
                    {sosActive && (
                        ...
                        >
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h2 className="text-3xl font-black uppercase tracking-widest mb-1">Emergency SOS</h2>
                                    <p className="text-red-100 font-medium text-lg">Smart Cane panic button triggered at {lastUpdated}</p>
                                </div>
                            </div>
                            <button
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    {/* Tracking Section */}
                    <div className={`lg:col-span-2 rounded-3xl p-6 md:p-8 flex flex-col ${sosActive ? 'bg-red-900 border border-red-800' : 'bg-white shadow-sm border border-slate-200'}`}>
                        <div className="flex items-center justify-between mb-6">
                            <h2 className="text-xl font-bold flex items-center gap-2">
                                <MapPin className={sosActive ? 'text-red-400' : 'text-blue-500'} />
                                Live Location Tracking
                            </h2>
                            {location && (
                                <div className="flex items-center gap-3">
                                    <div className="text-sm font-mono bg-slate-100 dark:bg-slate-800 px-3 py-1.5 rounded-lg text-slate-600 dark:text-slate-300">
                                    ...
                                    <a
                                        ...
                                        className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-sm font-bold transition-colors shadow-sm cursor-pointer ${sosActive
                        ...
                        <div className="flex-1 bg-[#f4f7fb] dark:bg-slate-900 rounded-2xl min-h-[400px] border border-slate-200 dark:border-slate-800 flex items-center justify-center overflow-hidden relative">
                            {location ? (
                                <iframe
                                    width="100%"
                                    height="100%"
                    ...
                    <div className="space-y-6">
                        ...
                        {/* Smart Hat Controls */}
                        <div className={`rounded-3xl p-6 border ${sosActive ? 'bg-red-900 border-red-800' : 'bg-white shadow-sm border-slate-200'}`}>
                            ...
                            <div className="space-y-3">
                                <Link
                                    ...
                                    className={`w-full py-4 px-4 rounded-xl flex items-center justify-center gap-3 font-semibold transition-all active:scale-[0.98] ${sosActive
                        ...
                        {/* Hardware Status */}
                        <div className={`rounded-3xl p-6 border ${sosActive ? 'bg-red-900 border-red-800' : 'bg-white shadow-sm border-slate-200'}`}>
                            <h2 className="text-lg font-bold mb-4">Device Status</h2>
                            <div className="space-y-4">
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className="text-xs font-mono bg-[#f4f7fb] px-2 py-1 rounded text-slate-500">Connected</span>
                                </div>
                                <div className="flex items-center justify-between">
                                    <span className="text-sm font-medium flex items-center gap-2">
                                        <div className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.8)]"></div>
                        ...
                        <div className={`rounded-3xl p-6 border ${sosActive ? 'bg-red-900 border-red-800' : 'bg-white shadow-sm border-slate-200'}`}>
                            ...
                            <div className="space-y-2 h-48 flex flex-col-reverse overflow-y-auto bg-[#f4f7fb] dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 text-xs font-mono">
                                {apiLogs.length === 0 ? (
                                    ...
                                    apiLogs.map((log, i) => (
                                        <div key={i} className="flex gap-3 border-b border-slate-200 dark:border-slate-800 pb-1 mt-1">
                                            ...
                                            <span className="text-emerald-600 dark:text-emerald-400 break-all">{log.message}</span>
                                        </div>
                                    ))
                                )}
                            </div>
